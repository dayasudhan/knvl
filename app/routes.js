...
Firebase.initializeApp({
  serviceAccount: {
    ...
    client_email:
      "khaanavali@project-8598805513533999178.iam.gserviceaccount.com",
    client_id: "110281937967415310229",
    auth_uri: "https://accounts.google.com/o/oauth2/auth",
    token_uri: "https://accounts.google.com/o/oauth2/token",
    auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
    client_x509_cert_url:
      "https://www.googleapis.com/robot/v1/metadata/x509/khaanavali%40project-8598805513533999178.iam.gserviceaccount.com",
  },
...
module.exports = function (app, passport) {
  ...
  app.get("/", function (req, res) {
    if (req.isAuthenticated()) res.render("customer", { user: req.user });
    else res.render("customer", { user: "dummy" });
  });
  ...
  function registerCustomer2(req, res, next) {
    ...
    var result = getNextSequence("customer", function (data) {
      ...
      return CustomerInfoModel.findOneAndUpdate(
        { phone: phoneNumber },
        {
  ...
  app.post("/v1/customer/address/:id", function (request, response) {
    ...
    if (
      checkVendorApiAunthaticated(request, 2) == false &&
    ...
    return CustomerInfoModel.update(
      { phone: phoneNumber },
      {
        $addToSet: {
          addresses: {
  ...
  app.get("/v1/customer", function (request, response) {
    return CustomerInfoModel.find(
      { phone: request.user.local.email },
      function (err, customerInfo) {
        if (!err) {
          return response.send(customerInfo);
        } else {
  ...
  app.delete(
    "/v1/customer/address/:phone/:label",
    function (request, response) {
      ...
      return CustomerInfoModel.update(
        ...
        { $pull: { addresses: { label: request.params.label } } },
        function (err) {
          if (!err) {
  ...
  app.patch("/v1/customer/address/:phone/:label", function (request, response) {
    ...
    return CustomerInfoModel.update(
      { phone: request.params.phone, "addresses.label": request.params.label },
      {
        $set: {
          "addresses.$.addressLine1": request.body.address.addressLine1,
  ...
  app.get("/v1/vendor/info/:id", function (request, response) {
    ...
    if (
      checkVendorApiAunthaticated(request, 1) == false &&
      request.isAuthenticated() == false
    ) {
      return response.send("Not aunthiticated").status(403);
    }
    return VendorInfoModel.find(
      { "hotel.email": request.params.id },
      function (err, vendor) {
        if (!err) {
          console.log(vendor);
  ...
  app.post("/v1/vendor/review/:id", function (req, res) {
    ...
    VendorInfoModel.update({ "hotel.id": req.params.id }, {}, function (err) {
      if (!err) {
        ...
        if (err) {
          ...
          otpModel.save(function (err) {
            if (!err) {
              ...
              var otpurl =
                "https://www.smsgatewayhub.com/api/mt/SendSMS?APIKey=mhgGNz6lk0ytziomd49mcQ&senderid=WEBSMS&channel=2&DCS=0&flashsms=0&route=1";
              //&number=919566229075&text=test message";
              otpurl = otpurl + "&number=" + req.body.phoneNumber;
              otpurl =
                otpurl +
                "&text=" +
                "Your one time password for khaanavali is : " +
                otpnum;
              console.log(otpurl);
              client.get(otpurl, function (data, response) {
                console.log(data);
  ...
  app.post("/v1/vendor/info/:id", function (req, res) {
    if (
      checkVendorApiAunthaticated(req, 1) == false &&
  ...
  function storeVendoruniqueId(request, response, callback, param) {
    ...
    console.log(request.params.id);
    VendorInfoModel.update(
      { "hotel.email": request.body.email },
      {
        ...
        function (err, vendor) {
          if (!err) {
            ...
            for (var j = 0; j < vendor.length; j++) {
              var googledistanceurl =
                "https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&key=AIzaSyA0ZqdBPzMZZBJ3O8bc5p-HtlQptpHIxJE";
              googledistanceurl =
                googledistanceurl +
                "&origins=" +
              ...
              googledistanceurl =
                googledistanceurl +
                "&destinations=" +
                vendor[j].address.latitude +
                "," +
                vendor[j].address.longitude;
                  ...
                  if (i == vendor.length) {
                    for (var k = 0; k < vendor2.length; k++) {
                      ...
                      for (var ii = 0; ii < menu_array.length; ii++) {
                        console.log(current_time);
                        if (
                          menu_array[ii].timings & current_time &&
                          menu_array[ii].availability == 1
                        ) {
          ...
          if (!err) {
            ...
            for (var j = 0; j < vendor.length; j++) {
              ...
              for (var i = 0; i < menu_array.length; i++) {
                if (
                  menu_array[i].timings & current_time &&
                  menu_array[i].availability == 1
                ) {
  ...
  function sendOrderReceivedSmstoVendor(order, phone, order_id) {
    ...
    for (var i = 0; i < order.menu.length; i++) {
      menuItms =
        menuItms + order.menu[i].name + "(" + order.menu[i].no_of_order + ") ";
    }
    console.log("menuItms", menuItms);
    var customerAddress = "";
    if (order.customer.address.addressLine1 != null) {
      customerAddress =
        customerAddress + order.customer.address.addressLine1 + "\n";
    }
    if (order.customer.address.areaName != null) {
      customerAddress =
        customerAddress + order.customer.address.areaName + "\n";
    }
    if (order.customer.address.addressLine2 != null) {
      customerAddress =
        customerAddress + order.customer.address.addressLine2 + "\n";
    }
    if (order.customer.address.LandMark != null) {
      customerAddress =
        customerAddress + order.customer.address.LandMark + "\n";
    }
    ...
    client.get(orderurl, function (data, response) {
      console.log(data);
    });
  }
    ...
    var res = getNextSequence("order", function (data) {
      ...
      order.save(function (err) {
        if (!err) {
          ...
          VendorInfoModel.find(
            { "hotel.email": order.hotel.email },
            function (err, vendor) {
              if (!err) {
                //update the pn
  ...
  app.post("/v1/admin/coverageArea", function (request, response) {
    ...
    var coverageArea = new CoverageAreaModel(dd);
  ...
  app.put("/v1/admin/coverageArea", function (request, response) {
    ...
    return CoverageAreaModel.update(
      { cityName: request.body.cityName },
      {
        $addToSet: {
          subAreas: {
            $each: [{ name: request.body.areaName, isBulkAreaOnly: isbulk }],
          },
        },
      },
      function (err, order) {
  ...
  app.put("/v1/admin/coverageArea/slider", function (request, response) {
    ...
    return CoverageAreaModel.update(
      ...
      {
        $set: {
          "sliders.logo1": request.body.logo1,
    ...
    return VendorInfoModel.find(
      ...
      function (err, vendorinfo) {
        if (!err) {
          console.log("no error");
          var menu_array;
          if (vendorinfo.length > 0) menu_array = vendorinfo[0].menu;
          else menu_array = vendorinfo;
  ...
  app.post("/v1/vendor/menu", function (request, response) {
    ...
    return VendorInfoModel.update(
      { "hotel.email": request.user.local.email },
      {
        $addToSet: {
          menu: {
            $each: [
              {
                name: request.body.fooditem,
  ...
  //Delete a menu item
  app.delete(
    "/v1/vendor/menu/item/:id/:fooditem",
    function (request, response) {
      ...
      return VendorInfoModel.update(
        { "hotel.email": request.params.id },
        { $pull: { menu: { name: request.params.fooditem } } },
        function (err) {
          if (!err) {
            console.log("Book removed");
            return response.send("");
  ...
  app.post("/v1/vendor/menu/item/:id", function (request, response) {
    ...
    return VendorInfoModel.update(
      { "hotel.email": request.params.id, "menu.name": request.body.fooditem },
      { $set: { "menu.$.availability": request.body.availability } },
      function (err) {
        if (!err) {
  ...
  app.get("/v1/admin/vendor", function (request, response) {
    console.log("/v1/admin/counters");
    // if (checkVendorApiAunthaticated(request, 0) == false) {
    //   return response.send("Not aunthiticated").status(403);
    // }
  ...
  app.post("/v1/admin/counters/:id", function (request, response) {
    ...
    var dd = { _id: request.params.id, sequence: 0 };
    var counters = new CountersModel(dd);
  ...
  async function getNextSequence(name, result) {
    ...
    var ret2 = await CountersModel.findOneAndUpdate(filter, update);
    console.log("ret_counters", ret2, name);
  ...
  function checkVendorApiAunthaticated(request, type) {
    ...
    var ret = false;
    if (
      request.headers.securekey == secureadminkey &&
      request.headers.client == client_key_admin
    ) {
      console.log("checkVendorApiAunthaticated admin");
      ret = true;
    } else if (
      request.headers.securekey == securewebkey &&
      ...
      if (
        request.headers.securekey == securevendorkey &&
        request.headers.version == version_value_1 &&
      ...
      if (
        request.headers.securekey == securecustomerkey &&
        request.headers.version == version_value_1 &&
